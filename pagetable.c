...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// dirty page (modified after creating) 

		
		if ((coremap[frame].pte->frame & PG_DIRTY) == PG_DIRTY) {
			

			// update pte to show that it is in swap, change PG SWAP to 1
			coremap[frame].pte->frame = coremap[frame].pte->frame | PG_ONSWAP ; // performs bitwise or 

			// copy to disk since dirty
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if ((p->frame & PG_VALID) != PG_VALID) {
	    ...
	    if ((p->frame & PG_ONSWAP) == PG_ONSWAP) {
	      int swap_status = swap_pagein(frame, p->swap_off);
	      if (swap_status != 0) {
	        swap_destroy();
	        exit(1);
	      }
